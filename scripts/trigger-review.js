 ...
 * 
 * 这个脚本不直接审阅，而是发送消息到 main-agent 的会话，
 * 让 main-agent 用 Sonnet 模型进行审阅。
 */
...
if (!taskId) {
  ...
  console.log('3. 输出审阅 Prompt（需要人工复制到 main-agent 会话）');
  process.exit(0);
}
...
console.log('');
console.log('📋 审阅 Prompt（复制到 main-agent 会话，用 Sonnet 模型）：');
console.log('='.repeat(50));
console.log(prompt);
console.log('='.repeat(50));
console.log('');
...
console.log('  在 main-agent 会话中：');
console.log('  sessions_spawn({');
console.log('    task: `<上面的 prompt>`,');
console.log('    model: "advanced-model/sonnet",');
console.log('    mode: "run"');
console.log('  })');
console.log('  然后用 apply-review.js 应用结果');
console.log('');
console.log('方式 2：手动 - 复制 Prompt 到 main-agent 会话，保存结果为 JSON 文件');
console.log('  1. 复制上面的 Prompt');
console.log('  2. 在 main-agent 会话中发送（确保用 advanced-model/sonnet 模型）');
